...
| `crackhash.request-ttl-millis` | TTL запроса: по истечении незавершённый запрос переводится в `ERROR`, слот планировщика освобождается |
...
| `crackhash.request-history-capacity` | Максимум записей в памяти по `requestId`                                        |
...
| `ERROR` | Ошибка, таймаут, переполнение очереди, запрос не найден (в т.ч. вытеснен из истории) |
...
- История `requestId` и кэш ограничены; старые завершённые записи могут быть вытеснены — повторный `GET` по такому `requestId` вернёт `ERROR`.
- После таймаута manager перестаёт учитывать запрос как активный, но он может висеть на worker, делать еще мониторинг этого я не буду
